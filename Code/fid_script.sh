...
current_iter=$3
for i in $(seq 1 $1); do
	python fid.py "$SLURM_TMPDIR/Output/Extra/${i}" "${4}" -i "$SLURM_TMPDIR/models/Inception" --gpu "0" --output_name "${2}" --at $current_iter --output_dir "/scratch/jolicoea/Output/Extra/"
